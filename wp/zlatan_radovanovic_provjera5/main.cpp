...
const int STEP = 4;
const int RECT_WIDTH = 100, RECT_HEIGHT = 190;
...
void update(HWND hwnd) {
    ...
    if (tank.x == RECT_WIDTH && tank.y > (rect.bottom - RECT_HEIGHT - tank.height/2)
        || tank.x == rect.right - RECT_WIDTH - tank.width && tank.y > (rect.bottom - RECT_HEIGHT - tank.height/2)
        || tank.x >= rect.right - RECT_WIDTH - tank.width && tank.y >= (rect.bottom - RECT_HEIGHT - tank.height)
        || tank.x <= RECT_WIDTH - tank.width && tank.y >= (rect.bottom - RECT_HEIGHT - tank.height)) {
        explosions.push_back(Explosion(true));
    }
...
void draw(HDC hdc, HWND hwnd) {
    ...
    HBRUSH hbrush = CreateSolidBrush(RGB(0, 70, 245));
    GetClientRect(hwnd, &rect);
    FillRect(hdc_buffer, &rect, hbrush);
    hbrush = CreateSolidBrush(RGB(150, 250, 0));
    DeleteObject(SelectObject(hdc_buffer, hbrush));
    Rectangle(hdc_buffer, rect.left, rect.bottom - RECT_HEIGHT, RECT_WIDTH, rect.bottom);
    Rectangle(hdc_buffer, rect.right - RECT_WIDTH, rect.bottom - RECT_HEIGHT, rect.right, rect.bottom);
    DeleteObject(hbrush);
...
LRESULT CALLBACK WindowProcedure (HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    switch (message)                  /* handle the messages */
    {
        ...
        case WM_MOUSEMOVE:
            {
                int possibleX = LOWORD(lParam) - tank.width / 2;
                int possibleY = HIWORD(lParam) - tank.height / 2;

                if (possibleX >= rect.right - tank.width)
                    possibleX = rect.right - tank.width;
                else if (possibleX <= 0)
                    possibleX = 0;

                if (possibleY > rect.bottom - tank.height)
                    possibleY = rect.bottom - tank.height;
                else if (possibleY < tank.height)
                    possibleY = tank.height;

                if (possibleX <= RECT_WIDTH && (tank.y + tank.height) > (rect.bottom - RECT_HEIGHT)){
                    if (tank.y <= rect.bottom - RECT_HEIGHT) {
                        possibleY = rect.bottom - RECT_HEIGHT - tank.height;
                    } else {
                        possibleX = RECT_WIDTH;
                    }
                }
                /*if (possibleX >= rect.right - RECT_WIDTH - tank.width && tank.y > (rect.bottom - RECT_HEIGHT - tank.height))
                    possibleX = rect.right - RECT_WIDTH - tank.width;*/
